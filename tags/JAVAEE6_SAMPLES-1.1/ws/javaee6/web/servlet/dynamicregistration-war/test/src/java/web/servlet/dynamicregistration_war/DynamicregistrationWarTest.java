...
/**
 * junit test for dynamicregistration-war.
 *
 * @author Jan Luehe
 */
public class DynamicregistrationWarTest {

    private static final String REQUEST_URI = "/dynamicregistration_war/";
    ...
    @Test public void testWebClient() throws Exception {
        ...
        try {
            ...
            if (code != 200) {
                Assert.fail("dynamicregistration_war failed with response code: " + code);
            } else {
            ...
            Assert.fail("dynamicregistration_war failed with exception: " +
                        ex);
        } finally {
            if (input != null) {
                input.close();
