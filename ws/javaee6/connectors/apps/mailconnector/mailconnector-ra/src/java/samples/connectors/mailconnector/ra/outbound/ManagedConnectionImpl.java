...
public class ManagedConnectionImpl implements ManagedConnection
{
	...
	PasswordCredential pc = 
	    Util.getPasswordCredential(mcf, subject, cxRequestInfo);
	    
        if (!Util.isPasswordCredentialEqual(pc, passCred))
        {
            //logger.finest("getConnection: User authentication failed");
            throw new SecurityException(resource.getString("PRINCIPAL_DOES_NOT_MATCH"));
        }
	...
	// We only need the Folder name as all the connections share the store
        ...
        { 
            folderName = ((ConnectionRequestInfoImpl)cxRequestInfo).getFolderName();         
        } else {
            // Use default values
        ...
        try
	{
        ...
        JavaMailConnection javaMailCon = new JavaMailConnectionImpl(this, folder );
        ...
        if (connection instanceof JavaMailConnection) 
	{
            ...
            throw new IllegalStateException(resource.getString("INVALID_CONNECTION"));
        }
    }
    ...
    {
        if (destroyed) {
            throw new IllegalStateException(resource.getString("DESTROYED_CONNECTION"));
        }
    }
