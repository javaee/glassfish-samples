...
import org.glassfish.webcomm.WebCommunicationServletContextListener;
import org.glassfish.webcomm.annotation.WebHandler;
import org.glassfish.webcomm.api.ServerSentEventHandler;
import org.glassfish.webcomm.sse.ServerSentEventServlet;
 ...
 */
@HandlesTypes(WebHandler.class)
public class WebCommunicationServletContainerInitializer implements ServletContainerInitializer {

    public void onStartup(Set<Class<?>> set, ServletContext ctx) throws ServletException {
        if (set == null || set.isEmpty()) {
            return;
        }
        ctx.addListener(WebCommunicationServletContextListener.class);
        ...
        for(Class<?> clazz : set) {
            if (ServerSentEventHandler.class.isAssignableFrom(clazz)) {
                WebHandler handler = clazz.getAnnotation(WebHandler.class);
                if (handler == null) {
