...
package web.servlet.servletcontainerinitializer_war;
...
public class ServletcontainerinitializerWarTest {
    ...
    @Test public void testWebClient() throws Exception {
        ...
        try {
            ...
            if (code != 200) {
                Assert.fail("servletcontainerinitializer_war failed with response code: " + code);
            } else {
            ...
            Assert.fail("servletcontainerinitializer_war failed with exception: " +
                        ex);
        } finally {
            if (input != null) {
                input.close();
